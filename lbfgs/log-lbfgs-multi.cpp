...
bool LBFGS::evalWolfe() {
    ...
    return lossSum - preLossSum  <= lambda2 * stepSize * dot(lastGrad, direction, W_SIZE, pool, BATCH_SIZE);
}
...
void LBFGS::cal_and_save_ST() {
    ...
    scal(direction, 0 - stepSize, W_SIZE, pool, BATCH_SIZE);
    //direction->scal(stepSize);
	double* ss = direction;
	// grad = grad - lastGrad, lastGrad = lastGrad + grad
    //grad->pax(-1, *lastGrad);
    ypax(grad, -1, lastGrad, W_SIZE, pool, BATCH_SIZE);
    //lastGrad->pax(1, *grad);
    ypax(lastGrad, 1, grad, W_SIZE, pool, BATCH_SIZE);
	// grad = grad - lastGrad grad will be y
	double* y = grad;
	updateST(ss, y);
    H = dot(ss, y, W_SIZE, pool, BATCH_SIZE) / dot(y, y, W_SIZE, pool, BATCH_SIZE);
}
...
double* LBFGS::getDirection(double* qq) {
	...
	int k = min(m, s->size());
    if(k > 0) rho[k-1] = 1.0 / dot((*s)[k-1], (*t)[k-1], W_SIZE, pool, BATCH_SIZE);
	for (int i = k-1; i >= 0; --i) {
        alpha[i] = dot(q, (*s)[i], W_SIZE, pool, BATCH_SIZE) * rho[i];
        ypax(q, 0 - alpha[i], (*t)[i], W_SIZE, pool, BATCH_SIZE);
	}
    scal(q, H, W_SIZE, pool, BATCH_SIZE);
    //scalWithCondition(q, H, W_SIZE);
	for (int i = 0; i < k; ++i) {
        double beta = rho[i] * dot(q, (*t)[i], W_SIZE, pool, BATCH_SIZE);
        ypax(q, alpha[i] - beta, (*s)[i], W_SIZE, pool, BATCH_SIZE);
	}
    // shift rho
    if(k == m) {
        for(int i = 0; i < k-1; ++i) {
